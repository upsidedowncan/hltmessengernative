...
const SwipeUpButton = ({ 
  ...
  const Container = BlurView;
  const containerProps = { intensity: 20, tint: 'light' as const };
...
export default function CallScreenIOS() {
  ...
  useEffect(() => {
    if (isBottom.value) {
      const topOffset = 60;
      const bottomBase = SCREEN_HEIGHT - PIP_HEIGHT - topOffset - MARGIN - 30;
        ...
        <View style={styles.videoContainer}>
          ...
          {((!isSwapped && !hasRemoteVideo) || (isSwapped && !isVideoEnabled) || callStatus !== 'Connected') && (
             <View style={styles.infoContent}>
                ...
                {!isVideo && (
                  <View style={styles.iosAudioBadge}>
                    <Ionicons name="call" size={14} color="#fff" />
                    <Text style={styles.iosAudioText}>Audio Call</Text>
                  </View>
                )}
             </View>
          )}
        ...
        {!hasAnswered && (
          <SafeAreaView style={styles.incomingControls} pointerEvents="box-none">
             <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                ...
                <Text style={styles.incomingStatus}>{isIncoming ? 'Incoming call...' : 'Calling...'}</Text>
             </View>

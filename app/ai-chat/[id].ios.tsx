...
const VisualizationEmbed = React.memo(({ content }: { content: string }) => {
  ...
  return (
    <View style={{ height, width: embedWidth, marginVertical: 8, overflow: 'hidden', borderRadius: 12 }}>
      <WebView
        ref={webviewRef}
        originWhitelist={['*']}
...
const ReasoningAccordion = ({ content, isRunning, onToggle }: { content: string, isRunning?: boolean, onToggle?: () => void }) => {
  ...
  return (
    <View style={{ marginVertical: 8, borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: theme.border, width: '100%' }}>
      <TouchableOpacity 
        activeOpacity={0.7}
        onPress={() => {
        ...
        style={{ flexDirection: 'row', alignItems: 'center', padding: 12, backgroundColor: theme.cardBackground }}
      >
      ...
      >
         <View style={{ padding: 12, borderTopWidth: 1, borderTopColor: theme.border }}>
            <Text style={{ fontSize: 14, lineHeight: 20, color: theme.text }}>{content.trim()}</Text>
        </View>
      </View>
...
const ImageGenerationBlock = React.memo(({ content }: { content: string }) => {
  ...
  if (status === 'error') {
    return (
      <View style={{ padding: 12, backgroundColor: theme.cardBackground, borderRadius: 12, marginVertical: 8, borderWidth: 1, borderColor: '#FF3B30' }}>
        <Text style={{ color: '#FF3B30', fontSize: 14 }}>Failed to generate image.</Text>
  ...
  if (status === 'loading' || status === 'idle') {
    return (
      <View style={{ padding: 20, backgroundColor: theme.cardBackground, borderRadius: 12, marginVertical: 8, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: theme.border }}>
...
const PythonExecutionBlock = React.memo(({ content, isLastMessage, onExecutionComplete, onToggle }: { content: string, isLastMessage: boolean, onExecutionComplete: (output: string) => void, onToggle?: () => void }) => {
  ...
  return (
    <View style={{ marginVertical: 8, borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: theme.border, backgroundColor: theme.cardBackground, width: '100%' }}>
      <TouchableOpacity 
        activeOpacity={0.8}
        onPress={() => {
            setExpanded(!expanded);
        ...
        style={{ flexDirection: 'row', alignItems: 'center', padding: 12, backgroundColor: theme.border + '20', borderBottomWidth: expanded ? 1 : 0, borderBottomColor: theme.border }}
      >
        <Animated.View style={arrowStyle}>
        ...
        <View style={{ marginLeft: 'auto', flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            {(data.status || executing) && (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                    ...
                    {data.status === 'success' && !executing && <Ionicons name="checkmark-circle" size={16} color="#34C759" />}
                    {data.status === 'error' && !executing && <Ionicons name="alert-circle" size={16} color="#FF3B30" />}
                </View>
            )}
        </View>
      </TouchableOpacity>
      ...
      {data.output ? (
          <View style={{ padding: 12, borderTopWidth: 1, borderTopColor: theme.border }}>
              ...
              <Text style={{ fontFamily: 'Menlo', fontSize: 12, color: theme.text }}>
                  {data.output}
              </Text>
          </View>
      ) : null}
      ...
      {parts.map((part) => (
        <React.Fragment key={part.key}>
          {part.type === 'embed' ? (
            ...
            <View style={{ maxWidth: '75%', marginBottom: 2 }}>
              <MessageBubble
                isMe={isMe}
        ...
        <View style={[styles.inputWrapper, { backgroundColor: theme.background, borderTopColor: theme.border, paddingBottom: Math.max(insets.bottom, 6) }]}>
          ...
          <View style={styles.innerContainer}>
              ...
              <Host>
                <Button 
                  onPress={handleSend}
                  disabled={(loading && !inputText) || isLimitReached || !inputText.trim()} 
...
const styles = StyleSheet.create({
  ...
  bubble: { maxWidth: '100%', paddingHorizontal: 12, paddingVertical: 8, overflow: 'visible' },
  liquidGlassBubble: { maxWidth: '100%', padding: 0 },
